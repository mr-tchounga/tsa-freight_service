package com.shifter.freight_service.services;

import com.shifter.freight_service.payloads.responses.AuthUserResponse;
import com.shifter.freight_service.repositories.CategoryRepository;
import com.shifter.freight_service.utils.Utils;
import jakarta.transaction.Transactional;
import lombok.AllArgsConstructor;
import org.springframework.stereotype.Service;

import java.lang.reflect.Field;
import java.util.Calendar;
import java.util.List;
import java.util.Map;
import java.util.Optional;

@Service
@Transactional
@AllArgsConstructor
public class CategoryService implements EntityInterface<Category> {

    private final CategoryRepository categoryRepository;
    private Utils utils;

    @Override
    public Optional<Category> findEntityById(Long id, AuthUserResponse user) {
        Category entity = new Category();
        entity.setId(id);
        entity.setVisible(true);
        entity.setCreatedBy(user.getId());
        Map<String, Object> nonNullElements = utils.getNonNullProperties(Category.class, entity);
        nonNullElements.put("id", id);
        if (!user.getRole().getName().equals("ADMIN")) {
            nonNullElements.put("isVisible", true);
        }
        List<Category> entities = utils.findAllByCustomQuery(nonNullElements, Category.class);
        return entities.isEmpty() ? Optional.empty() : Optional.of(entities.getFirst());
    }

    @Override
    public List<Category> findAllEntity(AuthUserResponse user) {
        Category entity = new Category();
        entity.setVisible(true);
        entity.setCreatedBy(user.getId());
        Map<String, Object> nonNullElements = utils.getNonNullProperties(Category.class, entity);
        if (!user.getRole().getName().equals("ADMIN")) {
            nonNullElements.put("isVisible", true);
        }
        return utils.findAllByCustomQuery(nonNullElements, Category.class);
    }

    @Override
    public List<Category> findFilterAllEntity(AuthUserResponse user, Category entity) {
        entity.setCreatedBy(user.getId());
        Map<String, Object> nonNullElements = utils.getNonNullProperties(Category.class, entity);
        if (!user.getRole().getName().equals("ADMIN")) {
            nonNullElements.put("isVisible", true);
        }
        return utils.findAllByCustomQuery(nonNullElements, Category.class);
    }

    @Override
    public Category addEntity(AuthUserResponse user, Category entity) {
        try {
            entity.setCreatedBy(user.getId());
            entity.setCreatedAt(Calendar.getInstance().getTime());
            return  categoryRepository.save(entity);
        } catch (Exception e) {
            throw new RuntimeException("Error adding new category: " + e.getMessage());
        }
    }

    @Override
    public Category updateEntity(AuthUserResponse user, Category entity) {
        Optional<Category> previousEntity = categoryRepository.findById(entity.getId());

        if (previousEntity.isPresent()) {
            previousEntity.get().setUpdatedBy(user.getId());
            previousEntity.get().setUpdatedAt(Calendar.getInstance().getTime());
            Map<String, Object> nonNullElements = utils.getNonNullProperties(Category.class, entity);

            nonNullElements.forEach((key, value) -> {
                try {
                    Field field = utils.findFieldInHierarchy(Category.class, key);
                    // If not found, try converting 'isVisible' -> 'visible' (common mismatch)
                    if (field == null && key.startsWith("is") && key.length() > 2) {
                        String alt = Character.toLowerCase(key.charAt(2)) + key.substring(3);
                        field = utils.findFieldInHierarchy(Category.class, alt);
                    }
                    if (field == null) {
                        throw new NoSuchFieldException(key);
                    }
                    field.setAccessible(true);
                    field.set(previousEntity.get(), value);
                } catch (NoSuchFieldException | IllegalAccessException e) {
                    throw new RuntimeException("Error updating field: '" + key + "':\n" + e.getMessage());
                }
            });
            return categoryRepository.save(previousEntity.get());
        } else {
            throw new RuntimeException("Category with id '" + entity.getId() + "' not found");
        }
    }

    @Override
    public void deleteEntity(AuthUserResponse user, Long id) {
        try {
            Category entity = new Category();
            entity.setId(id);
            entity.setVisible(false);
            updateEntity(user, entity);
        } catch (Exception e) {
            throw new RuntimeException("Error deleting category with id '" + id + "': " + e.getMessage());
        }
    }


}
